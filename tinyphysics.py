...
class TinyPhysicsSimulator:
  ...
  def control_step(self, step_idx: int) -> None:
    ...
    # instead of providing just the current values, we provide the whole history
    ...
    action = self.controller.update(self.target_lataccel_history, self.current_lataccel, self.state_history, step_idx, future_plan=self.futureplan)
